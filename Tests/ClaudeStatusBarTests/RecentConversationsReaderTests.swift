...
final class RecentConversationsReaderTests: XCTestCase {
    ...
    // MARK: - parseFirstPrompt CLI 注入过滤

    func testParseFirstPromptSkipsLocalCommandCaveat() {
        // `!` 命令在 jsonl 里以伪 user 消息形式出现, 包了 <local-command-caveat>.
        // 应当跳过, 继续找下一条真 prompt.
        let data = #"""
    ...
    func testParseFirstPromptSkipsSlashCommandMessage() {
        // /xxx slash command 在 jsonl 里被 wrap 成 <command-name> + <command-message> + <command-args>.
        let data = #"""
    ...
    func testParseFirstPromptSkipsLocalCommandStdout() {
        // `!` 命令的 stdout 也以伪 user 消息回传.
        let data = #"""
        {"type":"user","message":{"role":"user","content":"<local-command-stdout>some output</local-command-stdout>"}}
        {"type":"user","message":{"role":"user","content":"prompt after stdout"}}
        """#.data(using: .utf8)!
        XCTAssertEqual(
            RecentConversationsReader.parseFirstPrompt(data),
            "prompt after stdout"
    ...
    func testParseFirstPromptReturnsNilWhenAllUserMessagesAreCliInjections() {
        // 整个 jsonl 都是 CLI 注入(没有用户主动发的内容)→ 整体 nil → 候选会被 read 跳过.
        let data = #"""
        {"type":"user","message":{"role":"user","content":"<local-command-caveat>x</local-command-caveat>"}}
        {"type":"user","message":{"role":"user","content":"<command-name>/init</command-name>"}}
        {"type":"user","message":{"role":"user","content":"<local-command-stdout>y</local-command-stdout>"}}
        """#.data(using: .utf8)!
        XCTAssertNil(RecentConversationsReader.parseFirstPrompt(data))
    }
